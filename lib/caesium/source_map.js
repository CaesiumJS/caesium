...
FileTypes = require('./file_types.json')
...
SourceMap.prototype.buildObjects = function () {
  ...
  return this.iterate(function(fileObject, done){
    fs.readFile(path.join(_.map.base, fileObject.descriptor), function(err, data){
      ...
      fileObject.fileType = FileTypes[fileObject.ext]
...
SourceMap.prototype.buildPaths = function () {
  _ = this
  return this.iterate(function(fileObject, resolve, reject){
    if(typeof fileObject.fileType == 'undefined'){
      reject('No file type for ' + fileObject.descriptor)
    }

    handler = require("./handlers/" + fileObject.fileType.handler)

    fileObject.paths = handler.getPaths(fileObject)
    if(!_.parseList[fileObject.fileType.parseWeight]){
      _.parseList[fileObject.fileType.parseWeight] = []
    }
    _.parseList[fileObject.fileType.parseWeight].push(fileObject)
    resolve()
  })
}
